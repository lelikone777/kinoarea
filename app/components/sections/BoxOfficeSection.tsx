...
export function BoxOfficeSection({ entries, language, tmdbEnabled = true }: BoxOfficeSectionProps) {
  ...
  const serializedFilters = useMemo(() => JSON.stringify({ period, language }), [period, language]);
  ...
  }, [serializedFilters, period, language, tmdbEnabled, entries]);
      ...
      <div className="relative">
        ...
        {items.length === 0 && !loading ? (
          <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-sm text-slate-300">
            TMDB не вернул данных для выбранных фильтров. Измени период.
          </div>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {items.map((entry, index) => {
              const imageKey = `${entry.id ?? entry.title}-${index}`;
              const imageSrc = failedImages[imageKey] ? "/placeholders/poster.svg" : entry.image;
                ...
                >
                  <div className="relative h-20 w-16 overflow-hidden rounded-xl">
                    <Image
                      ...
                      onError={() =>
                        setFailedImages((current) => (current[imageKey] ? current : { ...current, [imageKey]: true }))
                      }
                    />
                  </div>
