...
export default async function MoviePage({ params }: MoviePageProps) {
  ...
  return (
    <div className="relative min-h-screen bg-slate-950 text-slate-50">
      ...
      <main className="relative z-10 mx-auto max-w-6xl px-3 pb-20 pt-8 sm:px-5 sm:pb-24 sm:pt-10">
        ...
        {errorMessage || !movie ? (
          ...
          <div className="grid gap-5 rounded-2xl border border-white/10 bg-slate-900/60 p-4 sm:gap-6 sm:rounded-3xl sm:p-5 md:grid-cols-[280px_1fr] lg:grid-cols-[320px_1fr]">
            ...
            <div className="space-y-4">
              <div>
                <h1 className="text-2xl font-extrabold sm:text-3xl">{movie.title}</h1>
              ...
              <div className="grid gap-2 text-sm text-slate-300 sm:grid-cols-2">
                <p>
                  <span className="text-white">IMDb:</span> {movie.imdbRating} ({movie.imdbVotes})
                </p>
                <p>
                  <span className="text-white">Режиссёр:</span> {movie.director}
                </p>
                <p>
                  <span className="text-white">Сценарий:</span> {movie.writer}
                </p>
                <p>
                  <span className="text-white">Актёры:</span> {movie.actors}
                </p>
                <p>
                  <span className="text-white">Премьера:</span> {movie.released}
                </p>
                <p>
                  <span className="text-white">Сборы:</span> {movie.boxOffice}
                </p>
              </div>
