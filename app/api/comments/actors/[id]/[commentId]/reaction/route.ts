...
export async function POST(request: Request, { params }: RouteContext) {
  ...
  const comment = await db.actorComment.findUnique({
    where: { id: commentId },
    select: { actorId: true },
  });

  if (!comment || comment.actorId !== actorId) {
    return NextResponse.json({ error: "Comment not found" }, { status: 404 });
  }

  if (!value) {
    await db.actorCommentReaction.deleteMany({
      where: { commentId, userId: session.userId },
    });
    return NextResponse.json({ ok: true });
  }

  await db.actorCommentReaction.upsert({
