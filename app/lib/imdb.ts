...
function getOmdbApiKey() {
  const key = process.env.OMDB_API_KEY?.trim() || process.env.NEXT_PUBLIC_OMDB_API_KEY?.trim();
  if (!key) {
    throw new Error("OMDB API key is not set. Provide OMDB_API_KEY (or NEXT_PUBLIC_OMDB_API_KEY).");
  }
  return key;
}
